...
class SDBAdditional:
    ...
    def find_target_instrument(self, search_string: str, is_shortname: bool = False):
        ...
        for entry in search_sdb:
            ...
            if entry['isAbstract']:
                ...
                if entry.get('shortName') is not None:
    ...
    async def load_execution_to_route(self):
        ...
        accounts = [
            acc for symbol
            in tree
            if symbol['brokers'] is not None
            for acc in symbol['brokers'].get('accounts', [])
            if acc['account'].get('executionSchemeId')
        ]
        wo_scheme = [
            acc for symbol
            in tree
        ...
        always_wo_scheme = {
            ...
            if len([
                acc for acc
                in accounts
                if acc['accountId'] == x['accountId'] 
            ]) < 10 # usually it should be zero BUT SOMETIMES
        }
        for scheme in await self.get_list_from_sdb(SdbLists.EXECSCHEMES.value):
            using_accounts = {
                x['accountId'] for x
                in accounts
                if x['account']['executionSchemeId'] == scheme[1]
            }
            item = (
                scheme[0],
                scheme[1],
                [
                    next(x for x in self.sdb_accs if x['_id'] == acc) for acc
                    in using_accounts
                ]
            )
            self.execution_to_route.append(item)
        self.execution_to_route.append((
            None,
            None,
            [
                next(x for x in self.sdb_accs if x['_id'] == acc) for acc
