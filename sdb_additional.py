...
from libs.symboldb_auth import SymbolDB
...
class SDBAdditional:
    ...
    def __cached_lists(self):
        collect_lists = []
        for num, l in enumerate(SdbLists):
            if num == 8:
                break
    ...
    def browse_folders(self, input_path: list, message: str = None, only_folders: bool = False) -> str:
        ...
        :param only_folders: don't show non-abstract instrumentshttps://apply.workable.com/exante/j/809789B48A/
        ...
        parents = []
        tree = self.load_tree(fields=['expiryTime'])
        ...
        while True:
            ...
            all_heirs = sorted(
                [deepcopy(x) for x in tree if len(x['path']) > 1 and x['path'][-2] == parents[-1]['_id']],
                key=lambda e: e['name']
            )
    ...
    def build_inheritance(self, payload: Union[str, dict, list], include_self=False) -> dict:
        """
        ...
        if isinstance(payload, str):
            instrument = self.sdb.get(payload)
        ...
        if not parents and not instrument.get('_id'):
            parents = sorted([self.sdb.get(x) for x in instrument['path']], key=lambda p: len(p['path']))
        elif not parents:
            parents = sorted(self.sdb.get_parents(instrument['_id']), key=lambda p: len(p['path']))[:-1]
        if include_self:
            exclude = []
    ...
    def compile_expiry_time(self, instrument, compiled=False):
        if not compiled:
            compiled_instrument = self.build_inheritance(instrument, include_self=True)
        else:
    ...
    def find_target_instrument(self, search_string: str, is_shortname: bool = False):
        ...
        tree = self.load_tree(
            fields=['expiryTime', 'exchangeId', 'ticker', 'description', 'type']
        )
        first_level = [
        ...
        # another option is to directly specify the instrument uuid
            ...
            if not tree or not next((x for x in tree if x.get('symbolId')), None):
                tree = self.load_tree(
                fields=['expiryTime', 'exchangeId', 'ticker', 'symbolId', 'description', 'type']
            )
            ...
            for entry in search_sdb:
                ...
                if entry['isAbstract']:
                    ...
                    if entry.get('description') is not None:
                        ...
                        if is_shortname:
                            match = re.search(rf'{input_path[0].lower()}', entry['description'].lower())
    ...
    def get_instrument_type(self, symbol) -> str:
        ...
        """
        tree = self.load_tree(fields=['type', 'symbolId'])
        if isinstance(symbol, str):
            ...
            return None
        parents = [next(x for x in tree if x['_id'] == y) for y in reversed(path)]
        return next((x['type'] for x in parents if x.get('type')), None)
    ...
    def get_list_from_sdb(self, list_name, id_only=True, additional_fields=[]) -> list:
        ...
        def check_cache():
            ...
            self.sdb_accs = self.sdb.get_broker_accounts() if not self.sdb_accs else self.sdb_accs
            self.sdb_gws = self.sdb.get_feed_gateways() if not self.sdb_gws else self.sdb_gws
            self.sdb_scheds = self.sdb.get_schedule() if not self.sdb_scheds else self.sdb_scheds
            self.sdb_currencies = self.sdb.get_currencies() if not self.sdb_currencies else self.sdb_currencies
    ...
    def isexpired(self, symbol) -> bool:
        ...
        """
        if isinstance(symbol, str):
            instr = self.sdb.get(symbol, fields=['expiryTime'])
    ...
    def load_execution_to_route(self):
    ...
    def load_tree(self, fields: list = [], reload_cache: bool = False) -> list:
        ...
        if 'symbolId' in fields:
            ...
            v2_fields = []
        loaded_tree_df = pd.DataFrame(self.sdb.get_tree(fields=fields))
        ...
        if v2_fields:
            get_v2_df = pd.DataFrame(self.sdb.get_v2('.*', fields=v2_fields))
            get_v2_df.set_index(
                '_id',
                drop=True,
                inplace=True
    ...
    def lua_compile(self, instrument: dict, template: str, compiled=False):
        ...
        if compiled_instrument.get('symbolId') is None and compiled_instrument.get('_id'):
            tree = self.load_tree(fields=['symbolId', 'expiryTime'])
    ...
    def show_path(self, path) -> str:
        ...
        if isinstance(path, list):
            return " → ".join([self.uuid_to_name(x)[0] for x in path])
            
        elif isinstance(path, str):
            get_instr = self.sdb.get(path)
            if not get_instr:
                return None
            ...
            if not get_path:
                return None
        return " → ".join([self.uuid_to_name(x)[0] for x in get_path])
    ...
    def uuid_to_name(self, uuid) -> tuple:
        ...
        tree_reload = False
        while True:
            tree = self.load_tree(reload_cache=tree_reload)
    ...
    def fancy_dict(
        ...
        **kwargs):
        ...
        always_transform = {
            ...
            'providerId': self.get_list_from_sdb(SdbLists.FEED_PROVIDERS.value)\
                        + self.get_list_from_sdb(SdbLists.BROKER_PROVIDERS.value),
